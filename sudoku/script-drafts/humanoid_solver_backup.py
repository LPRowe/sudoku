...
class sudoku(object):
    ...
    def solve_intelligent(self,arr):
        ...
        if len(arr.updated_possible_values)>0:
            ...
            self.history_intelligent=complete_intelligent_history(solved_sudoku.history_intelligent,solved_sudoku)
            return True
        else:
        ...
        for val in arr.updated_possible_values[(x,y)]:
            ...
            if self.solve_intelligent(arr=arr.arr):
                return True
...
if __name__=='__main__':
    ...
    test_extreme=False
    if test_extreme:
        #Use algorithm to solve the worlds-hardest sudokus
        sudoku_number=1 #pick a sudoku numbered between 0 and 17
        basefile='./puzzles/worlds-hardest/'
        names=[name.split("\\")[-1].split('.')[0] for name in glob.glob(basefile+'*')]
        arr=np.ndarray.astype(np.genfromtxt('./puzzles/worlds-hardest/'+names[1]+'.txt',delimiter=' '),'int')
    ...
    def save_puzzle(array,name,difficulty):
        f=open('./puzzles/'+difficulty+'/'+name+'.txt','w')
        row_count=0
        for row in array:
            count=0
            for value in row:
    ...
    if create_puzzles:
        for i in range(10):
            ...
            puzzle_count=len(glob.glob('./puzzles/easy/*'))
            ...
            #Check how many values are given in each difficulty level
            print(np.sum(s.easy_puzzle!=0))
            print(s.easy_puzzle)
            
            save_puzzle(s.easy_puzzle,'sudoku_easy_'+str(puzzle_count),'easy')
            
            print(np.sum(s.medium_puzzle!=0))
            print(s.medium_puzzle)
            
            save_puzzle(s.medium_puzzle,'sudoku_medium_'+str(puzzle_count),'medium')
            
            
            print(np.sum(s.hard_puzzle!=0))
            print(s.hard_puzzle)
            
            save_puzzle(s.hard_puzzle,'sudoku_hard_'+str(puzzle_count),'hard')
            
            
            print(np.sum(s.expert_puzzle!=0))
            print(s.expert_puzzle)
            
            save_puzzle(s.expert_puzzle,'sudoku_expert_'+str(puzzle_count),'expert')
    
        
            #solve the expert puzzle
